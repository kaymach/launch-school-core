...
counter = 0
# split full names into an array seperating the first and last
loop do
  break if counter == mailing_campaign_leads.size
  full_name = mailing_campaign_leads[counter][:name]
  names = full_name.split
# put first name into its own array and capitalize it
  names_counter = 0
  loop do
...
# put the now capitalized first and last name back together and modify hash to have it correct now
...
usable_leads = []
counter = 0
# capture last login and check if they're subscribed to the mailing list. if they logged in less than 60 days
# ago and theyre subscribed, they are put in the usable leads array.
loop do
  break if counter == mailing_campaign_leads.size
